...
public class ForgeControllerScreen extends AbstractContainerScreen<ForgeControllerMenu> {
    ...
    private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
        if(menu.isCrafting()) {
            guiGraphics.blit(RenderType::guiTextured, TEXTURE, x, y, 176, 14, menu.getArrowProgress(), 16, 256, 256);
        }
    }

    private void burnProgress(GuiGraphics guiGraphics, int x, int y) {
        if(menu.isBurning()) {
            guiGraphics.blit(RenderType::guiTextured, TEXTURE, x, y + 13 - menu.getBurnProgress(), 179, 13 - menu.getBurnProgress(), 9, menu.getBurnProgress(), 256, 256);
        }
    }
